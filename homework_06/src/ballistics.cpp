...
DropSolution compute_drop_solution(const BallisticsInput& input)
{
  ...
  float h = input.attack_speed * t - t * t * d * input.attack_speed / (2 * m) +
            std::pow(t, 3) * (6 * d * g * l * m - 6 * d * d * (l * l - 1) * input.attack_speed) / (36 * m * m) +
            std::pow(t, 4) *
              (-6 * d * d * g * l * (1 + l * l + std::pow(l, 4)) * m + 3 * std::pow(d, 3) * l * l * (1 + l * l) * input.attack_speed +
               6 * std::pow(d, 3) * std::pow(l, 4) * (1 + l * l) * input.attack_speed) /
              (36 * std::pow((1 + l * l), 2) * std::pow(m, 3)) +
            std::pow(t, 5) * (3 * std::pow(d, 3) * g * std::pow(l, 3) * m - 3 * pow(d, 4) * l * l * (1 + l * l) * input.attack_speed) /
              (36 * (1 + l * l) * std::pow(m, 4));

  float D = std::sqrt(std::pow(input.target_x - drone_x, 2) + std::pow(input.target_y - drone_y, 2));

  if (h + input.acceleration_path > D) {
    if (D < EPS) {
      drone_x = drone_x - h + input.acceleration_path;
      D = std::sqrt(std::pow(input.target_x - drone_x, 2) + std::pow(input.target_y - drone_y, 2));
    }
